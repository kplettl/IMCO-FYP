...
# label smoothing
    ...
    def train(self, config):
        ...
        self.writer = tf.summary.FileWriter("./logs/scalars/" + datetime.now().strftime("%Y%m%d-%H%M%S"), self.sess.graph)
    ...
    def complete(self, config):
        ...
        for idx in xrange(0, batch_idxs):
            ...
            save_images(batch_images[:batchSz,:,:,:], [nRows,nCols],
                        os.path.join(config.outDir, 'before/before' + datetime.now().strftime("_%Y%m%d-%H%M%S") + '.png'))
...
#                if i % config.outInterval == 0:
                if i  == 950:
                    ...
                    imgName = os.path.join(config.outDir,
                                           'hats_imgs/{:04d}'.format(i)+ datetime.now().strftime("_%Y%m%d-%H%M%S") + '.png')
                    nRows = np.ceil(batchSz/8)
                    nCols = min(8, batchSz)
                    ...
                    if i == 950:
                        imgName = os.path.join(config.outDir,
                                           'completed/{:04d}'.format(i) + datetime.now().strftime("_%Y%m%d-%H%M%S") + '.png')
