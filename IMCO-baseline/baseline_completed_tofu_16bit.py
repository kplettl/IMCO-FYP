# -*- coding: utf-8 -*-
"""
Created on Tue Dec 17 15:59:15 2019
...
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\outputImages\completed\completed-16bit-tofu-50epochs-baseline\completed\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    completed_images.append(im_)
    completed_images_array_50 = np.array(completed_images)
    
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\171219\16bit_tofu_test_set_baseline-recentered\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
...
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\outputImages\completed\completed-16bit-tofu-100epochs-baseline\completed\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    completed_images.append(im_)
...
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\outputImages\completed\completed-16bit-tofu-150epochs-baseline\completed\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    completed_images.append(im_)
...
mean_temp_50_16bit=[]
for i in range(0,16):
    mean_temp_50_16bit.append(np.mean(unscaled_completed_50epochs_16bit[i, 16:48, 16:48]))
 
mean_temp_100_16bit=[]
for i in range(0,16):
    mean_temp_100_16bit.append(np.mean(unscaled_completed_100epochs_16bit[i, 16:48, 16:48]))
    
mean_temp_150_16bit=[]
for i in range(0,16):
    mean_temp_150_16bit.append(np.mean(unscaled_completed_150epochs_16bit[i, 16:48, 16:48]))
...
average_baseline_temp_tofu_16bit_100 = []
baseline_temp_STD_tofu_16bit_100 = []
for i in range(0,16):
    average_baseline_temp_tofu_16bit_100 = np.append(average_baseline_temp_tofu_16bit_100, np.average(deltaT100[i, 31:44, 35:47]))
    baseline_temp_STD_tofu_16bit_100 = np.append(baseline_temp_STD_tofu_16bit_100,np.std(deltaT100[i,  31:44, 35:47]))
baseline_avg_tofu_16bit_100 = np.average(average_baseline_temp_tofu_16bit_100)   
baseline_std_tofu_16bit_100 = np.std(baseline_temp_STD_tofu_16bit_100)
...
for i in range (0,16):
    deltaTavgs.append(np.average(deltaT150[i,16:48,16:48], axis=0))
...
avg_baseline_temp_16bit_tofu = np.average(deltaTavgs__)
sd_baseline_temp_16bit_tofu = np.std(deltaTavgs__)
