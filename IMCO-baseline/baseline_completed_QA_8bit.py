...
phaseMapsCropped4 = phaseMaps[:,49:113, 50:114]
...
phaseMapsCropped5 = phaseMaps[:,49:113, 50:114]
...
''' Importing IMages ''' 
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\outputImages\completed\completed-8bitQA-baseline-50epochs-100120\completed\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    completed_images.append(im_)
    completed_images_array_50 = np.array(completed_images)
    
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\100120\8bit_QA_baseline_test_recentered_notincentre\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
...
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\outputImages\completed\completed-8bitQA-baseline-100epochs-100120\completed\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    completed_images.append(im_)
...
for filename in sorted(glob.glob(r"C:\Users\plettlk\DCGAN_Image_Completion\outputImages\completed\completed-8bitQA-baseline-150epochs-100120\completed\*.png"), key=numericalSort): 
    im=cv2.imread(filename, -1)
    im_ = cv2.cvtColor(im, cv2.COLOR_BGR2GRAY)
    completed_images.append(im_)
...
average_baseline_temp_qa_8bit_100 = []
baseline_temp_STD_qa_8bit_100 = []
for i in range(0,13):
    average_baseline_temp_qa_8bit_100 = np.append(average_baseline_temp_qa_8bit_100, np.average(deltaT100[i, 25:39, 27:37]))
    baseline_temp_STD_qa_8bit_100 = np.append(baseline_temp_STD_qa_8bit_100,np.std(deltaT100[i,  25:39, 27:37]))
baseline_avg_qa_8bit_100 = np.average(average_baseline_temp_qa_8bit_100)   
baseline_std_qa_8bit_100 = np.std(baseline_temp_STD_qa_8bit_100)
...
#plt.title('Similarity Measures of Original and Completed \n 8-bit Stationary QA Phantom Baseline Phase Images', size=18)
...
avg_baseline_temp_8bit_qa = np.average(deltaTavgs__)
sd_baseline_temp_8bit_qa = np.std(deltaTavgs__)
